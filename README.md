...
<img src="https://github.com/user-attachments/assets/7f1869b2-36ac-48fc-932a-8950e47cef0b" width="500px">
...
- **AI 자동 해시태그 생성 로직**: 게시글 작성 시 유저가 직접 태그를 입력하는 번거로움을 줄이기 위해, OpenAI API가 본문 내용을 분석하고 가장 연관성 높은 키워드를 추출하여 자동으로 태그를 매핑하는 시스템을 구축했습니다.
- **이모지 기반 커뮤니티 인터랙션**: 텍스트 위주의 댓글 외에도 15종의 감정 이모지 스티커 기능을 구현하여, 유저 간의 심리적 유대감을 높이고 직관적인 감정 공유가 가능하도록 설계했습니다.
- **상태 변화에 따른 동적 캐릭터 UI**: 사용자의 오늘의 감정 지수(Mood) 데이터를 실시간으로 반영하여 메인 페이지의 캐릭터 표정이 5단계로 변화하도록 구현, 서비스의 '힐링' 컨셉을 시각적으로 극대화했습니다.
- **데이터 정렬 및 검색 최적화**: 일주일간 가장 많이 사용된 실시간 인기 해시태그 순위를 산출하는 로직을 구현하고, 제목/작성자/태그별 복합 검색 기능을 통해 커뮤니티의 정보 접근성을 높였습니다.
